...
export class StrategyEngine extends EventEmitter {
  ...
  private strategyLoop: NodeJS.Timeout | null = null;
  ...
  processMarketData(marketData: MarketData): void {
    ...
    try {
      ...
      this.emit('error', new TradingError(`Strategy processing failed: ${error.message}`, 'STRATEGY_ERROR'));
    }
  }
   ...
   */
  private generateSignals(marketData: MarketData): StrategySignal[] {
   ...
   */
  private createArbitrageSignal(marketData: MarketData, threshold: number): StrategySignal | null {
    const marketId = marketData.marketId;
    const positions = this.state.positions.get(marketId) || [];
    ...
    const expectedProfit = ethers.BigNumber.from(Math.floor(inefficiency * 1e18));
    ...
    const signal: StrategySignal = {
      ...
      expectedProfit: expectedProfit.mul(positionSize).div(ethers.BigNumber.from(10).pow(18)),
   ...
   */
  private generateMomentumSignal(marketData: MarketData): StrategySignal | null {
   ...
   */
  private generateMeanReversionSignal(marketData: MarketData): StrategySignal | null {
   ...
   */
  private generateOpportunitySignal(opportunity: Opportunity): StrategySignal | null {
    ...
    return {
      ...
      expectedProfit: opportunity.expectedProfit.mul(positionSize).div(ethers.BigNumber.from(10).pow(18)),
   ...
   * Check for hedge opportunities
   */
  private checkHedgeOpportunities(marketData: MarketData): void {
    const marketId = marketData.marketId;
    const positions = this.state.positions.get(marketId) || [];
   ...
   */
  private needsHedging(marketId: string, marketData: MarketData): boolean {
    const positions = this.state.positions.get(marketId) || [];
    const lastHedgeTime = this.state.lastHedgeTime.get(marketId) || 0;
    const now = Date.now();
    
    // Check timeout
    if (now - lastHedgeTime > config.risk.hedgeTimeout) {
      return true;
    }
  ...
  private generateHedgeSignal(
    ...
    let signal: 'HEDGE_UP' | 'HEDGE_DOWN';
    let hedgeSize: ethers.BigNumber;
    
    if (upPositions.length > 0 && downPositions.length === 0) {
      // Need to hedge UP with DOWN
      signal = 'HEDGE_DOWN';
      hedgeSize = upPositions.reduce((sum, p) => sum.add(p.size), ethers.BigNumber.from(0));
    } else if (downPositions.length > 0 && upPositions.length === 0) {
      // Need to hedge DOWN with UP
      signal = 'HEDGE_UP';
      hedgeSize = downPositions.reduce((sum, p) => sum.add(p.size), ethers.BigNumber.from(0));
    } else {
    ...
    return {
      ...
      expectedProfit: ethers.BigNumber.from(0), // Hedges are for risk management
      risk: marketData.spread,
      timestamp: Date.now(),
  ...
  private calculatePositionSize(
    ...
    if (config.strategy.positionSizing === 'FIXED') {
      return config.strategy.baseSize;
    }
    
    if (config.strategy.positionSizing === 'KELLY') {
      ...
      return config.strategy.baseSize.mul(Math.floor(safeKelly * 10000)).div(10000);
    }
   ...
   * Update market state
   */
  private updateMarketState(marketData: MarketData): void {
   ...
   * Update performance metrics
   */
  private updateMetrics(marketData: MarketData): void {
  ...
  private initializeMetrics(): PerformanceMetrics {
    return {
      ...
      averageWin: ethers.BigNumber.from(0),
      averageLoss: ethers.BigNumber.from(0),
      profitFactor: 0,
