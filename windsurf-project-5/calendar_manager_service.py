...
class CalendarManagerService:
    def __init__(self, credentials_file='credentials.json'):
        self.auth = GoogleCalendarServiceAuth(credentials_file)
        self.service = None
    ...
    def sync_events(self, events: List[Dict], options: Dict = None) -> Dict:
        """
        Sincroniza eventos da planilha com o calendário usando Service Account
        """
        if not self.service:
            ...
            for i, event in enumerate(sorted_events):
                try:
                    ...
                    if created_event:
                        ...
                        # Compartilha calendário com participantes se for o primeiro evento
                        if result['created'] == 1 and event.get('participantes'):
                            for participant in event['participantes']:
                                self.auth.share_calendar_with_user(self.calendar_id, participant)
                    else:
                        result['errors'] += 1
                        result['details'].append({
                            'event': event['summary'],
