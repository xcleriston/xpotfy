...
Sistema para automatizar inserção de eventos, planejar deslocamento e sinalizar dias de folga
"""
...
class CalendarIntegrator:
    ...
    def _get_default_config(self):
        ...
        return {
            "spreadsheet": {
                "default_file": "data/eventos.xlsx",
                "auto_backup": True,
    ...
    def run(self, spreadsheet_file=None, options=None):
        ...
        try:
            ...
            print("🔄 Sincronizando com Google Agenda...")
    ...
    def create_sample_spreadsheet(self, output_path):
        """Cria uma planilha de exemplo"""
        try:
            created_path = self.spreadsheet_reader.create_sample_spreadsheet(output_path)
            print(f"✅ Planilha de exemplo criada: {created_path}")
            return True
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Integrador de Planilha com Google Agenda')
    ...
    parser.add_argument('--create-sample', help='Cria planilha de exemplo no caminho especificado')
    ...
    integrator = CalendarIntegrator(args.config)
    
    # Cria planilha de exemplo se solicitado
    if args.create_sample:
        integrator.create_sample_spreadsheet(args.create_sample)
        return
    ...
    print("🚀 Iniciando Integrador de Planilha com Google Agenda")
    print("="*60)
